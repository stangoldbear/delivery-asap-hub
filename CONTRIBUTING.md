...
```bash
python3.11 -c 'import dahub.view, dahub.gantt, dahub.api'
```
...
| change a colour or a metric | `settings.py`, and `dahub/static/theme.js` for the priority band, which is solved per theme |
...
- `node --check dahub/static/app.js`, and `theme.js` too, if you touched either.
